...
| `NEXT_PUBLIC_SUPABASE_ANON_KEY`                                              | **`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`**                                                                                         | Plan 01 Task 4 reads the publishable key, which is what `supabase status` now reports. Plan 02's snippet has been reconciled to `SUPABASE_PUBLISHABLE_KEY`, and its `createWorkerRpc` parameter renamed from `anonKey` to `publishableKey`.                                                                                                                                                                                                                                                                                                                                                                                                                             |
| A single `OPENAI_*` block covering analysis, vision and embeddings           | **Embeddings split onto their own provider: `EMBEDDING_BASE_URL`, `EMBEDDING_API_KEY`, `EMBEDDING_MODEL`, `EMBEDDING_DIMENSIONS`** | DeepSeek publishes no embeddings endpoint, so the vector half of hybrid search cannot come from the analysis provider. Verified: `POST api.deepseek.com/v1/embeddings` returns 404 and its model list contains no embedding model. Embeddings now run on Qwen via Alibaba Bailian, measured at 1024 dimensions with `text-embedding-v4`. `EMBEDDING_DIMENSIONS` is required with no default because migration `0006` declares `vector(N)` from it, and the adapter must always send it explicitly - the provider default happens to match today, which would hide a drift until every stored vector was the wrong width. Plans 02 and 03 are reconciled to these names. |
...
| `UploadSession.expiresAt` validated with `z.string().datetime()`             | **`z.string().datetime({ offset: true })`**                                                                                        | PostgREST renders `timestamptz` with a numeric offset (`+00:00`), and plain `.datetime()` accepts only a `Z` suffix. Every real upload batch would have failed schema parsing, been aborted, and surfaced only as the generic "无法签发上传授权". The task's own unit test used a `Z` literal, so it could not catch this; a regression test now uses the format PostgREST actually returns.                                                                                                                                                                                                                                                                            |
| The upload session route returning `error.message` for any failure           | **ZodError mapped to its first issue message**                                                                                     | A `ZodError`'s `message` is a JSON dump of its issues, so the browser received the validator's internals instead of the readable Chinese message the schema already defines.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `extensions.like(...)` is not a pgTAP function                                                                                                                                                     | `supabase/tests/0004_private_upload_sessions.test.sql`                                                                             | Use `extensions.alike(...)`, pgTAP's LIKE assertion. Same class as the `not_ok` defect.                                                                                                                                                                                                            |
| The `file.upload_completed` audit assertion runs as the uploading editor                                                                                                                           | `supabase/tests/0004_private_upload_sessions.test.sql`                                                                             | Read it as the owner. `audits_read_manager` requires `members.manage_basic`, which an editor does not have, so the assertion saw zero whether or not the row existed. The session assertion has to stay ahead of the role switch, because `upload_sessions_read_creator` is scoped to the creator. |
| `seedFoundationFixture` inserting workspaces then memberships through PostgREST                                                                                                                    | **Seeded in one transaction through `pg`**                                                                                         | Each PostgREST call is its own transaction, and the owner guard is a deferred constraint trigger, so the workspace insert commits alone with no owner membership and fails with `23514`. Verified directly against the running API before rewriting.                                               |
| `magicLink()` used to sign a browser in                                                                                                                                                            | **`signIn(page, email)` drives the real form and reads the emailed link**                                                          | An admin-generated link cannot establish a session here: the callback exchanges a PKCE code whose verifier only exists in the browser that submitted the form. Driving the real form is also what an acceptance gate should assert.                                                                |
| `onCompleted={() => undefined}` passed from the library Server Component                                                                                                                           | **`onCompleted` is optional and the page omits it**                                                                                | A Server Component cannot pass a function to a Client Component. As written, the library page threw `Event handlers cannot be passed to Client Component props` for every user, on both the empty and populated branches.                                                                          |
| Pages calling `requireWorkspaceCapability` directly                                                                                                                                                | **`requireWorkspaceOr404`**                                                                                                        | The layout guarded denial with `notFound()`, but pages render concurrently with the layout, so an uncaught throw in the page surfaced as a 500 error screen instead of a 404. An inaccessible workspace must be indistinguishable from a missing one.                                              |
| E2E asserting the transient in-dialog progress line                                                                                                                                                | **Asserts the queued document row, and that the dialog closed**                                                                    | A successful batch refreshes the route, which swaps the library between its empty state and its list and remounts the dialog, discarding the progress text. The dialog now closes deliberately rather than vanishing as a side effect.                                                             |
