...
This repository is a pnpm monorepo. [Plan 01 — Foundation](docs/superpowers/plans/2026-09-02-ai-knowledge-base-01-foundation.md) All eight tasks of Plan 01 are done: shared contracts are locked; the identity/workspace schema, the four-role capability matrix, RLS, and the audited membership RPCs are migrated and covered by 26 pgTAP assertions; email-OTP / Google sign-in, the session proxy, and the server-side permission gate are in place; the responsive workspace shell serves the library, graph, chat, and Q&A routes; teams can be created, members invited by email, and roles administered entirely through audited database RPCs; and a private Storage bucket accepts uploads only at database-minted, workspace-scoped object paths, with a browser upload flow covered end to end at 360, 768, 1024 and 1440 px. Plan 02 (ingestion and search) is next.
...
pnpm dev:login-link  # print the newest local sign-in link
pnpm db:status
pnpm db:reset
```
